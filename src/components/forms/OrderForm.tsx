...
function OrderForm({ orderStatus }: OrderFormProps) {
  ...
  return (
    <div className='flex gap-5 flex-wrap'>
        {Object.values(ORDER_STATUS_ADMIN_MAP).map(item => (
            <button className={`${selectedStatus === item.title ? "bg-gray-600" : "bg-transparent"} py-3 px-5 rounded-xl hover:bg-gray-600 cursor-pointer transition`} onClick={() => handleChangeStatus(item.title)}>
